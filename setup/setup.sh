...
printf "\033[1mUpdating crontab with: %s\033[0m\n" $(ls ${BASE_DIR}/etc/cron/{.header,*.cron})
cat ${BASE_DIR}/etc/cron/{.header,*.cron} | crontab -
printf "\033[1mUpdating root crontab with: %s\033[0m\n" $(ls ${BASE_DIR}/etc/cron/root/{.header,*.cron})
cat ${BASE_DIR}/etc/cron/root/{.header,*.cron} | sudo crontab -
